...
define void String_Add_String(%String* %this, %String* %other){
    ;Check second string size
    %var = alloca i32
    store i32 0, i32* %var
    %1 = getelementptr %String* %other, i32 0, i32 1
    %length = load i32* %1
    %bufptr = getelementptr %String* %this, i32 0, i32 0
    %buffer = load i8** %1
loop:
    %current_index = load i32, i32* %var
    %elem = getelementptr inbounds i8, i8* %buffer, i64 0, i64 %current_index
    call void @String_Add_Char(%this, %elem)
    %add_res = add nsw i32 %current_index, 1
    store i32 %add_res, i32* %var
    %isdone = icmp sle i32 %length, %add_res
...
define void @printString(%String* %str){
    call void @
    call void @printstr(i8* %buffer)
    ret void
}
define void @printstr(i8* %s) {
